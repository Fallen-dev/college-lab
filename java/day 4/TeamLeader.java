...
class TeamLeader extends ProdunctionWorker {
    private String bonus = "0";
    private int requiredTrainingHours, attendedTrainingHours;

    TeamLeader bonus(double bonus) {
        this.bonus = bonus + " /- per month";
        return this;
    }

    TeamLeader training(int requiredTrainingHours, int attendedTrainingHours) {
    ...
    void displayTL() {
        System.out.printf(
                "Bonus: %s\nRequired number of training hours: %d\nAttended number of training: %d\n",
                bonus, requiredTrainingHours, attendedTrainingHours);
    }
}
